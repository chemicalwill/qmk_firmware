...
def build(file, targets, name=DEFAULT_NAME, device=DEFAULT_DEVICE):
    ...
    for t, target in enumerate(targets):
        ...
        tdata = (
            struct.pack("<6sBI255s2I", b"Target", 0, 1, name, len(tdata), len(target))
            + tdata
        )
        data += tdata
...
%prog {-i|--build-ihex} file.hex [-i file.hex ...] [{-D|--device}=vendor:device] outfile.dfu"""
    ...
    parser.add_option(
        ...
        action="append",
        dest="binfiles",
        help="build a DFU file from given BINFILES. Note that the BINFILES must not have any DFU suffix!",
        metavar="BINFILES",
    )
    parser.add_option(
        "-i",
